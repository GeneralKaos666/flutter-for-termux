...
- If your engine build keeps failing in the Vulkan/SwiftShader parts, keep `[build].vulkan = false` in `build.toml` to pass `shell_enable_vulkan=false` and `skia_use_vulkan=false` during GN generation.
