...
python3 build.py patch --file=./patches/engine.patch                    # Apply Termux patches
...
2. **`utils.py __MODE__` is `('release', 'debug', 'profile')`** — release first. `Output.any` picks the first existing directory; this drives which dart-sdk snapshots the Flutter CLI finds.
...
python3 build.py patch --file=./patches/engine.patch  # May need patch updates if fails
```
3. Run full build
