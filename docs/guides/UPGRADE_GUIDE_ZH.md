[English](UPGRADE_GUIDE.md) | **中文**

# Flutter 版本升級指南

本文件說明如何將 Termux Flutter 從目前的 3.47.2 升級到新版本，並列出 3.47.2 之後必須重新檢查的 Dart / Flutter Tools / Gradle plugin 風險點。
...
□ Step 5: 套用 patch (engine / dart / skia)
□ Step 6: 組裝 sysroot
□ Step 7: configure + build (debug / release / profile)
□ Step 8: debuild 打包 .deb
□ Step 9: 推送到設備測試
□ Step 10: 更新 post_install.sh（如有必要）
□ Step 11: 發佈 GitHub Release
```
...
Patch 放在 `patches/` 扁平目錄（engine.patch、dart.patch、skia.patch），與版本無關。
...
<details>
<summary><b>ARM64-only APK — 安裝時 CLI 修改</b></summary>

`post_install.sh` 修改 Flutter CLI 的 `build_apk.dart` / `build_aar.dart` / `build_appbundle.dart` 及 Flutter Gradle plugin，
將預設 `targetPlatform` 改為只有 `[arm64]`（停用 arm/x64 gen_snapshot）。

> ⚠️ 這是安裝後在設備端套用，並非建構階段。
...
> ARM64-only APK 限制由安裝時的 `post_install.sh` 處理
> （透過 `build_apk`、`build_aar`、`build_appbundle` 與 `plugin_utils` 修補），
> 並非建構階段的 patch。
...
>
> ⚠️ **重要**：`utils.py` 裡的 `__MODE__` 元組是 `('release', 'debug', 'profile')`，
> release 在第一個位置。`Output.any` 會挑第一個存在的目錄，
> 因此 Flutter CLI 由 release (product mode) 的 dart-sdk snapshots 驅動。不要隨意調整模式順序。
...
```bash
# Dart binary 由標準構建的 flutter/build/archives:dart_sdk_archive 產出
# （FFT pipeline 沒有獨立的 build_dart 步驟）
```

> ✔️ `dart_sdk_archive`、`flutter_patched_sdk`、`flutter_gtk` 皆包含在 `build()` 的 ninja 調用中。

---

## Step 9: Android gen_snapshot

> FFT pipeline 的標準 `build()` 產出 Linux engine targets；Termux 上的 Android APK 構建使用 deb 包 artifacts 內附的 gen_snapshot。
...
`python3 build.py`（Build.__call__）會依序執行：
1. config
2. clone
3. sync
4. 每個 arch：sysroot → configure + build（debug → release → profile），包含 `dart_sdk_archive`
5. debuild
...
├── README.md                 # 英文主文檔
├── README_ZH.md              # 中文翻譯
├── docs/guides/UPGRADE_GUIDE.md # ← 本文件（英文版）
