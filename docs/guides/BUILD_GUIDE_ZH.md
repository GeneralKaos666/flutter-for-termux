...
| 補丁檔案 | 解決的問題 |
|----------|-----------|
| `patches/engine.patch` | Bionic TLS 對齊、`-llog -lm` 連結、dynamic linker 路徑、NDK clang runtime 偵測 |
| `patches/dart.patch` | Dart VM / profiler shutdown Termux 適配 |
...
### 根本原因
**僅 `ninja flutter` 不會編譯 dart 二進制！**

deb 包中 `bin/cache/dart-sdk/bin/dart` 二進制文件必須存在，否則：
- flutter 命令無法正確執行
- gen_snapshot 版本不匹配錯誤

### 解決方案
FFT 的 `build()` ninja 調用包含 `flutter/build/archives:dart_sdk_archive`（以及 `flutter_patched_sdk`、`flutter_gtk`），因此 dart 二進制隨標準構建產出：
...
**解決方案：**
確保執行完整構建流程，讓 dart 與 gen_snapshot 來自同一次構建：
```bash
python3 build.py
```
...
| 功能 | 狀態 | 說明 |
|------|------|------|
| `flutter --version` | ✅ 正常 | Flutter 3.47.2 / Tools Dart 3.13.2 |
| `dart --version` | ✅ 正常 | Termux JIT Dart 3.13.2 (`android_arm64`) |
| `dartvm --version` | ✅ 正常 | post-install `dartvm` resolves to Dart 3.13.2 (`android_arm64`) |
...
```bash
# patches 放在扁平目錄 patches/（engine.patch、dart.patch、skia.patch）
python3 build.py patch --file=./patches/engine.patch
