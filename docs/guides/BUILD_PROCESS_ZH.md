...
- `patches/dart.patch` - Dart SDK / VM Termux 適配
- `patches/skia.patch` - Skia Android/bionic build 適配

ARM64-only APK 限制由安裝時的 `post_install.sh` 處理，並非建構階段的 patch。
...
**重要**：`build()` 的 ninja 調用包含 `flutter/build/archives:dart_sdk_archive`，因此 dart 二進制隨標準構建產出（無需單獨步驟）。

### 階段 5：構建 Android gen_snapshot（可選，用於 APK 構建）

> 標準構建會將 gen_snapshot 放入 deb artifacts，無需單獨的 Android configure/build 步驟。
...
├── patches/ # Flutter Engine / Dart / Skia 補丁
├── build.py              # 主構建腳本
