...
#   5. Build test APK
#
# Once finished, you can use flutter build apk to build any project
#
...
configure_ndk_clang() {
	...
	# Skip the empty NDK stub (empty dir shipped with the android-sdk package)
	if [ ! -d "$PREBUILT/linux-x86_64/bin" ]; then
		echo "  Skipping NDK stub: $(basename "$NDK_DIR")"
		return
	fi

	echo "  Configuring NDK: $(basename "$NDK_DIR")"
	...
	# Create the correct symlinks for each API level
	# Important: libc++_shared.so must point to the real library in the parent dir, not a linker script
	for api_dir in "$LIB_DIR"/*; do
		if [ -d "$api_dir" ]; then
			# libc++_shared.so - point to the real library file in the parent dir
			...
			# libatomic.a - create an empty stub (Android does not need libatomic)
	...
	# Patch android-legacy.toolchain.cmake (remove -static-libstdc++ to avoid link errors)
...
echo "║                                                           ║"
echo "║     The world's first native ARM64 Termux                 ║"
echo "║     solution for flutter build apk                        ║"
echo "║                                                           ║"
echo "╚═══════════════════════════════════════════════════════════╝"
echo -e "${NC}"
echo ""
...
echo "  • Total ~1.8GB"
echo ""
echo -e "${YELLOW}Estimated time:${NC} 10-30 minutes (depending on network speed)"
echo ""
...
# Remove any leftover old packages (avoid dependency conflicts)
...
# ========================================
echo ""
echo -e "${GREEN}[2/${TOTAL_STEPS}]${NC} Installing Flutter SDK..."
...
echo "Verifying Flutter SDK SHA256 checksum..."
...
echo "Verifying Android SDK SHA256 checksum..."
verify_sha256 "$ANDROID_SDK_DEB" "$ANDROID_SDK_EXPECTED_SHA256" || {
	INSTALL_FAILED=true
	record_stage integrity failed
	exit 30
}
...
NDK_PATH="$ANDROID_HOME/ndk/$NDK_VERSION"
if [ ! -d "$NDK_PATH" ]; then
	echo "Downloading ARM64 NDK..."
	wget -q --show-progress "$NDK_ARCHIVE_URL" -O "$NDK_ARCHIVE" || {
		INSTALL_FAILED=true
		record_stage download failed
		exit 20
	}
	echo "Verifying NDK SHA256 checksum..."
	verify_sha256 "$NDK_ARCHIVE" "$NDK_EXPECTED_SHA256" || {
		INSTALL_FAILED=true
		record_stage integrity failed
		exit 30
	}
...
# Pre-verify deb structure integrity (hard failure)
echo "Verifying Android SDK deb structure integrity..."
dpkg-deb --info "$ANDROID_SDK_DEB" >/dev/null 2>&1 || {
	INSTALL_FAILED=true
	record_stage integrity failed
	echo "Android SDK deb is corrupt"
	exit 30
}

# All downloads and verifications succeeded; start installation
echo "All downloads and verifications succeeded. Installing the SDK..."
MUTATION_STARTED=true

# ========================================
# Step 2: Install Android SDK (install-first, never purge-first)
# ========================================
echo ""
...
dpkg -i --force-architecture "$ANDROID_SDK_DEB" || dpkg --force-depends --configure android-sdk || {
	INSTALL_FAILED=true
...
# ========================================
echo ""
echo -e "${GREEN}[3/${TOTAL_STEPS}]${NC} Installing Flutter SDK..."
...
# Recompile flutter_tools.snapshot (fixes the "Unsupported operating system: android" issue)
...
if [ -f "$DART_SDK/bin/dart" ] && [ -f "$FLUTTER_ROOT/packages/flutter_tools/bin/flutter_tools.dart" ]; then
	echo "Recompiling flutter_tools.snapshot..."
...
# Download official Dart SDK snapshots (for hot reload / flutter run)
echo "Downloading Dart SDK snapshots..."
...
if [ -n "$ENGINE_VERSION" ] && [ ! -f "$SNAPSHOTS_DIR/dds_aot.dart.snapshot" ]; then
	...
	echo "  Downloading snapshots from official Flutter storage..."
	...
	echo "  ✓ Dart SDK snapshots already present or no update needed"
fi

# Clean ELF binaries (remove DT_RPATH warnings, fix flutter run JSON parse issue)
echo "Cleaning ELF binaries..."
mkdir -p "$WORK_DIR/apt_staging"
(
	cd "$WORK_DIR/apt_staging"
...
if command -v termux-elf-cleaner &>/dev/null; then
	...
	echo "  ⚠ termux-elf-cleaner not installed"
fi

# ========================================
# Step 4: Install ARM64 NDK
# ========================================
echo ""
echo -e "${GREEN}[4/${TOTAL_STEPS}]${NC} Installing ARM64 NDK..."

if [ -d "$NDK_PATH" ]; then
	echo "  ✓ NDK installed"
else
	echo "Extracting NDK (Staging Isolation)..."
	...
	echo "Moving NDK to target path..."
...
# Configure all installed NDKs
echo "Configuring NDK clang wrappers..."
...
# Also run post_install.sh (if present)
...
# Create build-tools symlinks (fix the "Build Tools is corrupted" error)
...
if ! command -v aapt2 &>/dev/null; then
	...
	(
		...
		# Install (use dpkg to avoid triggering apt dependency resolution)
...
# Create a project with Android and Linux support
...
# Configure project (ARM64 only + compileSdk 34)
...
# Gradle may have downloaded new SDK components (e.g. build-tools/35.0.0-2); reconfigure
echo "Configuring SDK components downloaded by Gradle..."
if [ -f "$PREFIX/share/flutter/post_install.sh" ]; then
	bash "$PREFIX/share/flutter/post_install.sh" || {
		INSTALL_FAILED=true
		record_stage post-install failed
...
# Check for failure due to NDK clang issues (Gradle may have downloaded a new NDK)
if grep -q "CMAKE_C_COMPILER" "$build1_log" 2>/dev/null || grep -q "compiler identification is unknown" "$build1_log" 2>/dev/null; then
	echo "Detected NDK clang issue, reconfiguring..."
...
# Test Linux build (if gtk3 installed)
LINUX_BUILD_SUCCESS=false
if command -v pkg-config &>/dev/null && pkg-config --exists gtk+-3.0 2>/dev/null; then
	echo "Building Linux app (requires gtk3)..."
	...
	if [ -f "build/linux/arm64/release/bundle/flutter_test_app" ]; then
		...
		echo "  ⚠ Linux build skipped (install gtk3 to enable)"
	fi
else
	echo "  ⚠ Linux build skipped (requires pkg install gtk3)"
fi
...
if [ "$BUILD_SUCCESS" = true ]; then
	echo -e "${CYAN}║     ${GREEN}Installation complete! APK build test successful!${CYAN}     ║${NC}"
fi
echo -e "${CYAN}╚═══════════════════════════════════════════════════════════╝${NC}"
echo ""

if [ "$BUILD_SUCCESS" = true ]; then
	echo -e "${GREEN}Test APK:${NC} $TEST_APP_DIR/build/app/outputs/flutter-apk/app-release.apk ($APK_SIZE)"
	echo ""
fi

echo -e "${YELLOW}Getting Started:${NC}"
echo ""
echo -e "${GREEN}Environment is configured automatically! Ready to use after restarting Termux.${NC}"
echo ""
echo "1. Check Flutter:"
